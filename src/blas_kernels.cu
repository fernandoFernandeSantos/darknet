...
__global__ void scale_bias_kernel(real_t_device *output, real_t_device *biases, int n,
		int size) {
...
void scale_bias_gpu(real_t_device *output, real_t_device *biases, int batch, int n,
		int size) {
...
__global__ void backward_scale_kernel(real_t_device *x_norm, real_t_device *delta, int batch,
	...
	for (b = 0; b < batch; ++b) {
		for (i = 0; i < size; i += BLOCK) {
			...
			sum += (p + i < size) ? delta[index] * x_norm[index] : real_t_device(0);
		}
	}
	part[p] = sum;
	__syncthreads();
...
void backward_scale_gpu(real_t_device *x_norm, real_t_device *delta, int batch, int n,
		int size, real_t_device *scale_updates) {
	backward_scale_kernel<<<n, BLOCK>>>(x_norm, delta, batch, n, size,
			scale_updates);
	check_error(cudaPeekAtLastError());
}

__global__ void add_bias_kernel(real_t_device *output, real_t_device *biases, int batch,
		int n, int size) {
...
__global__ void backward_bias_conn_kernel(real_t_device *bias_updates, real_t_device *delta,
		int batch, int n) {
	int index = (blockIdx.x + blockIdx.y * gridDim.x) * blockDim.x
			+ threadIdx.x;
	if (index >= n)
...
__global__ void backward_bias_kernel(real_t_device *bias_updates, real_t_device *delta,
...
void backward_bias_gpu(real_t_device *bias_updates, real_t_device *delta, int batch, int n,
		int size) {
	if (size == 1) {
...
__global__ void normalize_kernel(int N, real_t_device *x, real_t_device *mean,
	...
	x[index] = (x[index] - mean[f]) / (sqrt_real(variance[f] + real_t_device(.00001f)));
}

__global__ void normalize_delta_kernel(int N, real_t_device *x, real_t_device *mean,
		real_t_device *variance, real_t_device *mean_delta, real_t_device *variance_delta, int batch,
		int filters, int spatial, real_t_device *delta) {
	int index = (blockIdx.x + blockIdx.y * gridDim.x) * blockDim.x
			+ threadIdx.x;
	if (index >= N)
		return;
	...
	delta[index] = delta[index] * real_t_device(1.f) / (sqrt_real(variance[f] + real_t_device(.00001f)))
			+ variance_delta[f] * real_t_device(2.f) * (x[index] - mean[f]) / real_t_device(spatial * batch)
			+ mean_delta[f] / real_t_device(spatial * batch);
}

//extern "C"
void normalize_delta_gpu(real_t_device *x, real_t_device *mean, real_t_device *variance,
		real_t_device *mean_delta, real_t_device *variance_delta, int batch, int filters,
		int spatial, real_t_device *delta) {
...
__global__ void variance_delta_kernel(real_t_device *x, real_t_device *delta, real_t_device *mean,
		real_t_device *variance, int batch, int filters, int spatial,
		real_t_device *variance_delta) {
...
__global__ void accumulate_kernel(real_t_device *x, int n, int groups, real_t_device *sum) {
	int k;
	int i = (blockIdx.x + blockIdx.y * gridDim.x) * blockDim.x + threadIdx.x;
	if (i >= groups)
		return;
...
__global__ void fast_mean_delta_kernel(real_t_device *delta, real_t_device *variance,
	...
	if (id == 0) {
		...
		mean_delta[filter] *= (real_t_device(-1.f) / sqrt_real(variance[filter] + real_t_device(.00001f)));
	}
}

__global__ void fast_variance_delta_kernel(real_t_device *x, real_t_device *delta,
		real_t_device *mean, real_t_device *variance, int batch, int filters, int spatial,
		real_t_device *variance_delta) {
	const int threads = BLOCK;
	__shared__ real_t_device local[threads];
	...
	for (j = 0; j < batch; ++j) {
		for (i = 0; i < spatial; i += threads) {
			...
			local[id] +=
					(i + id < spatial) ?
							delta[index] * (x[index] - mean[filter]) : real_t_device(0);
		}
	}
...
__global__ void mean_delta_kernel(real_t_device *delta, real_t_device *variance, int batch,
...
//extern "C"
void mean_delta_gpu(real_t_device *delta, real_t_device *variance, int batch, int filters,
		int spatial, real_t_device *mean_delta) {
...
//extern "C"
void fast_mean_delta_gpu(real_t_device *delta, real_t_device *variance, int batch,
		int filters, int spatial, real_t_device *mean_delta) {
...
//extern "C"
void fast_variance_delta_gpu(real_t_device *x, real_t_device *delta, real_t_device *mean,
		real_t_device *variance, int batch, int filters, int spatial,
		real_t_device *variance_delta) {
...
__global__ void mean_kernel(real_t_device *x, int batch, int filters, int spatial,
		real_t_device *mean) {
...
__global__ void variance_kernel(real_t_device *x, real_t_device *mean, int batch, int filters,
...
__global__ void reorg_kernel(int N, real_t_device *x, int w, int h, int c, int batch,
		int stride, int forward, real_t_device *out) {
	int i = (blockIdx.x + blockIdx.y * gridDim.x) * blockDim.x + threadIdx.x;
	if (i >= N)
		return;
	int in_index = i;
...
__global__ void scal_kernel(int N, real_t_device ALPHA, real_t_device *X, int INCX) {
	int i = (blockIdx.x + blockIdx.y * gridDim.x) * blockDim.x + threadIdx.x;
	if (i < N)
		X[i * INCX] *= ALPHA;
}

__global__ void fill_kernel(int N, real_t_device ALPHA, real_t_device *X, int INCX) {
	int i = (blockIdx.x + blockIdx.y * gridDim.x) * blockDim.x + threadIdx.x;
	if (i < N)
		X[i * INCX] = ALPHA;
}

__global__ void copy_kernel(int N, real_t_device *X, int OFFX, int INCX, real_t_device *Y,
		int OFFY, int INCY) {
	int i = (blockIdx.x + blockIdx.y * gridDim.x) * blockDim.x + threadIdx.x;
	if (i < N)
...
__global__ void mul_kernel(int N, real_t_device *X, int INCX, real_t_device *Y, int INCY) {
	int i = (blockIdx.x + blockIdx.y * gridDim.x) * blockDim.x + threadIdx.x;
	if (i < N)
		Y[i * INCY] *= X[i * INCX];
}
...
//extern "C"
void normalize_gpu(real_t_device *x, real_t_device *mean, real_t_device *variance,
...
__global__ void l2norm_kernel(int N, real_t_device *x, real_t_device *dx, int batch,
...
//extern "C"
void l2normalize_gpu(real_t_device *x, real_t_device *dx, int batch, int filters,
		int spatial) {
...
__global__ void fast_mean_kernel(real_t_device *x, int batch, int filters, int spatial,
		real_t_device *mean) {
	const int threads = BLOCK;
	__shared__ real_t_device local[threads];
...
__global__ void fast_variance_kernel(real_t_device *x, real_t_device *mean, int batch,
	...
	for (j = 0; j < batch; ++j) {
		for (i = 0; i < spatial; i += threads) {
			...
			local[id] +=
					(i + id < spatial) ? pow_real((x[index] - mean[filter]), 2) : real_t_device(0);
		}
	}
...
//extern "C"
void fast_variance_gpu(real_t_device *x, real_t_device *mean, int batch, int filters,
		int spatial, real_t_device *variance) {
...
//extern "C"
void variance_gpu(real_t_device *x, real_t_device *mean, int batch, int filters, int spatial,
		real_t_device *variance) {
...
//extern "C"
void copy_gpu_offset(int N, real_t_device * X, int OFFX, int INCX, real_t_device * Y,
		int OFFY, int INCY) {
...
//extern "C"
void flatten_gpu(real_t_device *x, int spatial, int layers, int batch, int forward,
		real_t_device *out) {
...
__global__ void smooth_l1_kernel(int n, real_t_device *pred, real_t_device *truth,
...
//extern "C"
void smooth_l1_gpu(int n, real_t_device *pred, real_t_device *truth, real_t_device *delta,
		real_t_device *error) {
	smooth_l1_kernel<<<cuda_gridsize(n), BLOCK>>>(n, pred, truth, delta, error);
	check_error(cudaPeekAtLastError());
}

__global__ void softmax_x_ent_kernel(int n, real_t_device *pred, real_t_device *truth,
		real_t_device *delta, real_t_device *error) {
	int i = (blockIdx.x + blockIdx.y * gridDim.x) * blockDim.x + threadIdx.x;
	if (i < n) {
...
//extern "C"
void softmax_x_ent_gpu(int n, real_t_device *pred, real_t_device *truth, real_t_device *delta,
		real_t_device *error) {
...
__global__ void logistic_x_ent_kernel(int n, real_t_device *pred, real_t_device *truth,
		real_t_device *delta, real_t_device *error) {
	int i = (blockIdx.x + blockIdx.y * gridDim.x) * blockDim.x + threadIdx.x;
	if (i < n) {
		real_t_device t = truth[i];
...
//extern "C"
void logistic_x_ent_gpu(int n, real_t_device *pred, real_t_device *truth, real_t_device *delta,
		real_t_device *error) {
...
__global__ void l2_kernel(int n, real_t_device *pred, real_t_device *truth, real_t_device *delta,
		real_t_device *error) {
	int i = (blockIdx.x + blockIdx.y * gridDim.x) * blockDim.x + threadIdx.x;
	if (i < n) {
		real_t_device diff = truth[i] - pred[i];
...
__global__ void l1_kernel(int n, real_t_device *pred, real_t_device *truth, real_t_device *delta,
		real_t_device *error) {
	int i = (blockIdx.x + blockIdx.y * gridDim.x) * blockDim.x + threadIdx.x;
	if (i < n) {
		real_t_device diff = truth[i] - pred[i];
...
__global__ void wgan_kernel(int n, real_t_device *pred, real_t_device *truth, real_t_device *delta,
		real_t_device *error) {
	int i = (blockIdx.x + blockIdx.y * gridDim.x) * blockDim.x + threadIdx.x;
	if (i < n) {
...
//extern "C"
void wgan_gpu(int n, real_t_device *pred, real_t_device *truth, real_t_device *delta,
		real_t_device *error) {
	wgan_kernel<<<cuda_gridsize(n), BLOCK>>>(n, pred, truth, delta, error);
	check_error(cudaPeekAtLastError());
}

__global__ void weighted_sum_kernel(int n, real_t_device *a, real_t_device *b, real_t_device *s,
		real_t_device *c) {
	int i = (blockIdx.x + blockIdx.y * gridDim.x) * blockDim.x + threadIdx.x;
	if (i < n) {
...
__global__ void deinter_kernel(int NX, real_t_device *X, int NY, real_t_device *Y, int B,
		real_t_device *OUT) {
	int i = (blockIdx.x + blockIdx.y * gridDim.x) * blockDim.x + threadIdx.x;
	if (i < (NX + NY) * B) {
		int b = i / (NX + NY);
...
__global__ void inter_kernel(int NX, real_t_device *X, int NY, real_t_device *Y, int B,
		real_t_device *OUT) {
	int i = (blockIdx.x + blockIdx.y * gridDim.x) * blockDim.x + threadIdx.x;
	if (i < (NX + NY) * B) {
		int b = i / (NX + NY);
...
__global__ void weighted_delta_kernel(int n, real_t_device *a, real_t_device *b, real_t_device *s,
		real_t_device *da, real_t_device *db, real_t_device *ds, real_t_device *dc) {
	int i = (blockIdx.x + blockIdx.y * gridDim.x) * blockDim.x + threadIdx.x;
	if (i < n) {
		if (da)
			da[i] += dc[i] * s[i];
...
//extern "C"
void weighted_delta_gpu(real_t_device *a, real_t_device *b, real_t_device *s, real_t_device *da, real_t_device *db,
		real_t_device *ds, int num, real_t_device *dc) {
	weighted_delta_kernel<<<cuda_gridsize(num), BLOCK>>>(num, a, b, s, da, db,
			ds, dc);
	check_error(cudaPeekAtLastError());
}

__global__ void mult_add_into_kernel(int n, real_t_device *a, real_t_device *b, real_t_device *c) {
	int i = (blockIdx.x + blockIdx.y * gridDim.x) * blockDim.x + threadIdx.x;
	if (i < n) {
		c[i] += a[i] * b[i];
	}
...
__device__ void softmax_device(real_t_device *input, int n, real_t_device temp, int stride,
