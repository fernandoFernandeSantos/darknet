...
__global__ void adam_kernel(int N, real_t_device *x, real_t_device *m, real_t_device *v, real_t_device B1,
	...
	real_t_device mhat = m[index] / (1.f - powf(B1, t));
	real_t_device vhat = v[index] / (1.f - powf(B2, t));
...
__global__ void variance_delta_kernel(real_t_device *x, real_t_device *delta, real_t_device *mean,
	...
	variance_delta[i] *= -.5f
			* powf(variance[i] + .00001f, (real_t_device)(-3.f / 2.f));
}
...
__global__ void fast_variance_delta_kernel(real_t_device *x, real_t_device *delta,
	...
	if (id == 0) {
		...
		variance_delta[filter] *= -.5f
				* powf(variance[filter] + .00001f, (real_t_device)(-3.f / 2.f));
	}
}
...
__global__ void scal_kernel(int N, real_t ALPHA, real_t_device *X, int INCX) {
	int i = (blockIdx.x + blockIdx.y * gridDim.x) * blockDim.x + threadIdx.x;
	if (i < N)
		X[i * INCX] *= ALPHA;
}

__global__ void fill_kernel(int N, real_t ALPHA, real_t_device *X, int INCX) {
	int i = (blockIdx.x + blockIdx.y * gridDim.x) * blockDim.x + threadIdx.x;
	if (i < N)
		X[i * INCX] = ALPHA;
}
...
__global__ void smooth_l1_kernel(int n, real_t_device *pred, real_t_device *truth,
	...
	if (i < n) {
		...
		real_t_device abs_val = fabsf(diff);
		if (abs_val < 1) {
			error[i] = diff * diff;
			delta[i] = diff;
		} else {
...
__device__ void softmax_device(real_t_device *input, int n, real_t_device temp, int stride,
	...
	for (i = 0; i < n; ++i) {
		real_t_device e = expf(input[i * stride] / temp - largest / temp);
		sum += e;
		output[i * stride] = e;
	}
	for (i = 0; i < n; ++i) {
