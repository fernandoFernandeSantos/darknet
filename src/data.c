...
void correct_boxes(box_label *boxes, int n, real_t dx, real_t dy, real_t sx, real_t sy, int flip)
{
    int i;
    for(i = 0; i < n; ++i){
...
void fill_truth_swag(char *path, real_t *truth, int classes, int flip, real_t dx, real_t dy, real_t sx, real_t sy)
{
    char labelpath[4096];
...
void fill_truth_region(char *path, real_t *truth, int classes, int num_boxes, int flip, real_t dx, real_t dy, real_t sx, real_t sy)
{
    char labelpath[4096];
    find_replace(path, "images", "labels", labelpath);
    find_replace(labelpath, "JPEGImages", "labels", labelpath);
...
void fill_truth_iseg(char *path, int num_boxes, real_t *truth, int classes, int w, int h, augment_args aug, int flip, int mw, int mh)
{
    char labelpath[4096];
...
void fill_truth_mask(char *path, int num_boxes, real_t *truth, int classes, int w, int h, augment_args aug, int flip, int mw, int mh)
{
    char labelpath[4096];
    find_replace(path, "images", "mask", labelpath);
    find_replace(labelpath, "JPEGImages", "mask", labelpath);
...
void fill_truth_detection(char *path, int num_boxes, real_t *truth, int classes, int flip, real_t dx, real_t dy, real_t sx, real_t sy)
{
    char labelpath[4096];
    find_replace(path, "images", "labels", labelpath);
    find_replace(labelpath, "JPEGImages", "labels", labelpath);
...
data load_data_seg(int n, char **paths, int m, int w, int h, int classes, int min, int max, real_t angle, real_t aspect, real_t hue, real_t saturation, real_t exposure, int div)
{
...
data load_data_iseg(int n, char **paths, int m, int w, int h, int classes, int boxes, int div, int min, int max, real_t angle, real_t aspect, real_t hue, real_t saturation, real_t exposure)
{
    char **random_paths = get_random_paths(paths, n, m);
    int i;
    data d = {0};
...
data load_data_mask(int n, char **paths, int m, int w, int h, int classes, int boxes, int coords, int min, int max, real_t angle, real_t aspect, real_t hue, real_t saturation, real_t exposure)
{
    char **random_paths = get_random_paths(paths, n, m);
    int i;
    data d = {0};
...
data load_data_region(int n, char **paths, int m, int w, int h, int size, int classes, real_t jitter, real_t hue, real_t saturation, real_t exposure)
{
    char **random_paths = get_random_paths(paths, n, m);
    int i;
    data d = {0};
...
data load_data_detection(int n, char **paths, int m, int w, int h, int boxes, int classes, real_t jitter, real_t hue, real_t saturation, real_t exposure)
{
    ...
    for(i = 0; i < n; ++i){
        ...
        real_t new_ar = (orig.w + rand_uniform(-dw, dw)) / (orig.h + rand_uniform(-dh, dh));
...
data load_data_regression(char **paths, int n, int m, int k, int min, int max, int size, real_t angle, real_t aspect, real_t hue, real_t saturation, real_t exposure)
{
    if(m) paths = get_random_paths(paths, n, m);
    data d = {0};
    d.shallow = 0;
...
void get_next_batch(data d, int n, int offset, real_t *X, real_t *y)
{
    ...
    for(j = 0; j < n; ++j){
        int index = offset + j;
        memcpy(X+j*d.X.cols, d.X.vals[index], d.X.cols*sizeof(real_t));
        if(y) memcpy(y+j*d.y.cols, d.y.vals[index], d.y.cols*sizeof(real_t));
    }
}
